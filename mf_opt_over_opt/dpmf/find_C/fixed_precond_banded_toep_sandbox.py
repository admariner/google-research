...
"""Sandbox for DP noising matrix optimization with fixed Toeplitz preconditioners."""
...
class BandedToeplitzOptimizationStrategy(MatrixStrategy):
  """A strategy using Banded Toeplitz Optimization."""
  ...
  @staticmethod
  @functools.partial(jax.jit, static_argnames=['bands', 'alpha'])
  def generate_C_bands(params, bands, alpha):
    """Generates strictly valid banded Toeplitz matrix parameters."""
    if bands == 1:
      a = jnp.ones((1,), dtype=params.dtype)
      return a, jnp.linalg.norm(a, ord=2)

    k = alpha * jnp.tanh(params)
    a = jnp.ones((1,), dtype=params.dtype)
    for i in range(bands - 1):
      a_padded = jnp.pad(a, (0, 1))
      a_rev = jnp.pad(jnp.flip(a, axis=0), (1, 0))
      a = a_padded + k[i] * a_rev
  ...
  def build_noising_matrix(
    ...
    a, norm_a = self.generate_C_bands(params, self.bands, self.alpha)
    return memory_friendly_toeplitz_inverse(a, scale=norm_a)
...
def main(argv):
  ...
  strategy = BandedToeplitzOptimizationStrategy(T=_T.value, bands=_BANDS.value)
